...
class AccountManagementAnalyzer:
    ...
    def classification(self):
      ...
      categories = {
        "Food/Clothing": [
            "zomato", "swiggy", "flipkart", "groceries", "amazon", "myntra", "ajio",
    ...
    def trans_pred(self):
      ...
      predicted_balance = forecast_arima.iloc[-1]

      print(f"Current Balance: ₹{current_balance:.2f}")
      print(f"Predicted Balance (in {future_days} days): ₹{predicted_balance:.2f}")
      print("\nFuture Predictions (ARIMA):\n", prediction_df[['Date', 'ARIMA_Prediction','LSTM_Prediction']])
      
      def disp_res(current_balance,future_days,predicted_balance):
        injected_text = f"""
        <p>Current Balance: ₹{current_balance:.2f}</p>
        <p>Predicted Balance (in {future_days} days): ₹{predicted_balance:.2f}</p>
        """
        ...
        # Inject the text into the <div id="stats-target">
        new_html = html.replace('<div id="stats-target">', f'<div id="stats-target">\n{injected_text}')

        # Save to a new HTML file (or overwrite the same one if you want)
        with open("./templates/result.html", "w", encoding="utf-8") as file:
            file.write(new_html)

        print("Balance stats injected into output.html")
    
      print(disp_res(current_balance,future_days,predicted_balance))

      # Use best prediction (LSTM or ARIMA)
      '''predicted_income = prediction_df["LSTM_Prediction"].mean()
      print(predicted_income)
      predicted_expense = prediction_df["ARIMA_Prediction"].mean()
      ...
      arima_predictions = forecast_arima[:len(y_test)]


      msme_lstm = np.mean((actual_balances.flatten() - lstm_predictions.flatten()) ** 2) / np.mean(actual_balances.flatten() ** 2)
      msme_arima = np.mean((actual_balances.flatten() - arima_predictions) ** 2) / np.mean(actual_balances.flatten() ** 2)

      from sklearn.metrics import mean_absolute_error
      mae_lstm = mean_absolute_error(actual_balances, lstm_predictions)
      rmae_lstm = mae_lstm / np.mean(actual_balances)

      mae_arima = mean_absolute_error(actual_balances.flatten(), arima_predictions)
      rmae_arima = mae_arima / np.mean(actual_balances)
    ...
    # setting the classification dataset
      ...
      # final concusion statements summary of budget
      print("\n📊 AI-Based Budget Suggestion for Next Month")
      print(f"Predicted Income: ₹{predicted_income:.2f}")
      print(f"Predicted Expenses: ₹{predicted_expense:.2f}")
      print(f"Recommended Savings ({savings_ratio*100}%): ₹{dynamic_savings:.2f}")
      print(f"Essential Expenses (70%): ₹{essential_expense:.2f}")
      print(f"Non-Essential Expenses (30%): ₹{non_essential_expense:.2f}")
      print("\nExpense Breakdown by Category (Excluding Transfers):")
      print(category_expense)
      print("\nAdaptive Budget Allocation Based on Past Trends:")
      print(adaptive_allocation)
	  
      


      # alert if overspending on some v=category
      over_spending = adaptive_allocation[adaptive_allocation > category_expense]
      if not over_spending.empty:
          print("\n⚠️ You are overspending in these categories:")
          print(over_spending)

      if predicted_expense > predicted_income:
          overspending_amount = predicted_expense - predicted_income
          print(f"\n⚠️ Warning: Your predicted expenses exceed your income by ₹{overspending_amount:.2f}. Consider reducing discretionary spending.")
      pass
